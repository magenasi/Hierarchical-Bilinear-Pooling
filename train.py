...
from nets.hbp_model import HBPNet
from utils.callbacks import LossHistory
from utils.dataloader import DataGenerator, detection_collate
from utils.utils import download_weights, get_classes, weights_init, show_config, get_lr_scheduler, \
	set_optimizer_lr
...
if __name__ == '__main__':
	...
	# ------------------------------------------------------#
	#   所用模型种类：
	#   MobileNetV2、MobileNetV3Large、MobileNetV3Small、ShuffleNetV2_x1_0、GhostNet、ResNet50
	# ------------------------------------------------------#
	...
	#   定义模型
	# ----------------------------------------------------#
	model = HBPNet(backbone=backbone,
	               phi=phi,
	...
	if total_step <= wanted_step:
		...
		print(
			"\033[1;33;44m[Warning] 本次运行的总训练数据量为%d，Unfreeze_batch_size为%d，共训练%d个Epoch，"
			"计算出总训练步长为%d。\033[0m" % (num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
	...
	if True:
		...
		optimizer = {
			...
			'sgd': optim.SGD(model_train.parameters(), Init_lr_fit, momentum=momentum,
			                 nesterov=True)
		}[optimizer_type]
		...
		for epoch in range(Init_Epoch, UnFreeze_Epoch):
			...
			if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
				...
				#   获得学习率下降的公式
				# ---------------------------------------#
				lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit,
				                                     UnFreeze_Epoch)
			...
			fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step,
